...
public:
    ...
    /// Binds the theme to settings so "appearance.theme" drives it, and the theme
    /// writes back when toggled from the UI.
    void bindTo(config::Settings& settings);
...
private:
    ...
    config::Settings* m_settings = nullptr;
};
