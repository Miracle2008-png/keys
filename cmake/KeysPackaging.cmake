...
if(WIN32)
    ...
    # The wizard's own artwork, drawn from the same mark the application uses.
    #
    # Without these the installer ships NSIS's stock graphics - a grey banner
    # and a blue arrow - and the first thing anyone sees of Keys is a dialog
    # that looks like it belongs to no product in particular. The bitmaps are
    # generated by tools/make-installer-art.ps1 in the graphite the application
    # actually paints, sampled from a running window rather than converted from
    # the OKLCH source, so the two agree exactly.
    #
    # 24-bit BMP because MUI accepts nothing else, and because an alpha channel
    # composites as black over the wizard's dialog background.
    # Emitted through CPACK_NSIS_DEFINES rather than the CPACK_NSIS_MUI_*
    # variables: this CPack template hardcodes `!define MUI_HEADERIMAGE` and
    # never substitutes the bitmap paths, so setting those variables silently
    # does nothing. DEFINES is a block the template does substitute.
    #
    # NSIS wants Windows separators; the paths arrive from CMake with forward
    # slashes, so they are converted rather than written by hand.
    file(TO_NATIVE_PATH
        "${CMAKE_SOURCE_DIR}/resources/icons/generated/installer-header.bmp"
        keys_header_bmp)
    file(TO_NATIVE_PATH
        "${CMAKE_SOURCE_DIR}/resources/icons/generated/installer-side.bmp"
        keys_side_bmp)
    string(REPLACE "\\" "\\\\" keys_header_bmp "${keys_header_bmp}")
    string(REPLACE "\\" "\\\\" keys_side_bmp "${keys_side_bmp}")

    # The strip along the bottom says "Nullsoft Install System v3.12"
    # otherwise, which names the build tool rather than the product - the
    # equivalent of shipping with the compiler's name on the splash screen.
    set(CPACK_NSIS_DEFINES "RequestExecutionLevel user
!define MUI_HEADERIMAGE_BITMAP '${keys_header_bmp}'
!define MUI_HEADERIMAGE_UNBITMAP '${keys_header_bmp}'
!define MUI_WELCOMEFINISHPAGE_BITMAP '${keys_side_bmp}'
!define MUI_UNWELCOMEFINISHPAGE_BITMAP '${keys_side_bmp}'
BrandingText 'Keys ${PROJECT_VERSION}'")
